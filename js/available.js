...
$(function(){
	$(".Top .title").mouseover(function(){
		$(this).css({background:"#dcdcda"});
		$(".title .drop").css({display:"block",background:"#f4f4f4"});
	...
	$(".Top .title").mouseout(function(){
		$(this).css({background:""});
		$(".title .drop").css({display:"none"});
		$(".title .drop a").eq(0).mouseout(function(){
			$(this).html("YOHO!"),
			$(this).css({color:"#98aebc"})
		});
	...
	$(".Top .message .cliser").mouseover(function(){
		...
		$(".Top .message .cliser .service").css({
			display:"block",background:"#eaeceb",
		})
	});
	...
	$(".arrowRight").click(function(){
		index++;
		if(index == $(".figure li").length){
			index = 0;
		}
		$(".center .figure li").eq(index).fadeIn(500);
		$(".center .figure li").eq(index).siblings().fadeOut();
	});
	$(".arrowLeft").click(function(){
		index--;
		if(index == -1){
			index = $(".figure li").length-1;
		}
		$(".center .figure li").eq(index).fadeIn(500);
		$(".center .figure li").eq(index).siblings().fadeOut();
	});
	time = setInterval(function(){
		...
		$(".center .figure li").eq(index).fadeIn(500);
		$(".center .figure li").eq(index).siblings().fadeOut();
		$(".smallPic li").eq(index).children("div").css({
			display:"none"
		});
		$(".smallPic li").eq(index).siblings().children("div").css({
			display:"block"
		});
	},4000);
	let oDiv = $("<div>");
	$(".smallPic li").append(oDiv);
	$(".smallPic li").mouseout(function(){
		...
		time = setInterval(function(){
			...
			$(".smallPic li").mouseover(function(){
				...
				$(".center .figure li").eq($(this).index()).fadeIn(0);
				$(".center .figure li").eq($(this).index()).siblings().fadeOut();
				clearInterval(time);
			});
		},4000);
	});
	...
	setInterval(function(){
		if(subscripted==($(".popularListFigureLeft").length-1)){
			$(".popularListFigure").css({
				left:0,
			})
			subscripted = 0
		}
		subscripted++;
		$(".popularListFigure").stop().animate({
				left:-1150*subscripted
			},1000);
	},3500)
	// MORE
	$(".smallList .last").mouseover(function(){
		$(this).css({
			textDecoration: "underline"
		});
	});
	$(".smallList .last").mouseout(function(){
		$(this).css({
			textDecoration: "none"
		});
	});
	...
	$(".skip").click(function(){
		...
		xhr.open("GET","mutal.php?num="+$(this).index(),true);
		...
		function fun(str){
			let arr = str.split("&")
			localStorage.setItem("图片地址",arr[0])
			localStorage.setItem("name",arr[1])
			localStorage.setItem("价格",arr[2])
